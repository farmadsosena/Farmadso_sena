...
$buscarCodigo = mysqli_query($conexion, "SELECT * FROM medicos WHERE identificacionprofesional LIKE '%$resul%'");

if ($buscarCodigo) {
  if (mysqli_num_rows($buscarCodigo) > 0) {
    while ($fi = mysqli_fetch_assoc($buscarCodigo)) {
      $user= $fi["idusuario"];
      $idUser= mysqli_query($conexion,"SELECT * FROM usuarios WHERE idusuario='$user'");
      $us = mysqli_fetch_assoc($idUser);
      echo '<div class="mauso-medicaa" data-codigo="'.$fi["identificacionprofesional"].'"  data-nombre="'.$us["nombre"]. " ".$us["apellido"].'" data-ids="'.$fi["idmedico"].'">
