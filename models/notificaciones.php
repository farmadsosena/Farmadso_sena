...
// Verificar si la consulta fue exitosa y $datosUsuarioCompra no es nulo
if ($resultadoPacientecompra && $datosUsuarioCompra) {
    $nombreInvitado = $datosUsuarioCompra["nombre"];
    $apellidoInvitado = $datosUsuarioCompra["apellido"];
    $telefonoInvitado = $datosUsuarioCompra["telefono"];
    $direccionInvitado = $datosUsuarioCompra["direccion"];
        ...
        if ($resultadoMedi->num_rows > 0) {
            ...
            if ($resultadoIdFarmacia->num_rows > 0) {
                ...
                if ($resultadoInfoFarmacia->num_rows > 0) {
                    ...
                    echo '<hr />';
                    echo '<div class="customerData">';
                    echo '<span>Dirección: B/' . $direccionInvitado . '</span>';
                    echo '<span>Cliente: ' . $nombreInvitado . ' ' . $apellidoInvitado . ' </span>';
                    echo '<a href="tel: + ' . $telefonoInvitado . '">' . $telefonoInvitado . '</a>';
                    echo '</div>';
                    echo '<div class="buttonSeeMore" onclick="abrirNoti(' . $datosCompra["idcompra"] . ')">';
                    echo '<a href="#" class="seeMore" >Ver más</a>';
                    echo '</div>';
